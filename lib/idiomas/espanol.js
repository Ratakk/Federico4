...
const smsRowner = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO YO COMO CREADOR DE BOT LO PUEDE USAR!!\`\`\`` }
...
const smsPremium = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO ESTA DISPONIBLE PARA MI CREADOR!!\`\`\`` }//USUARIOS PREMIUM
...
const smsSaludo = () => { return `BIENVENIDO`}
...
const smsLista1 = () => { return `INFORMACIÓN DE FEDERICO`}
...
const smsLista7 = () => { return `INSTALAR FEDERICO`}
...
const smsLista24 = () => { return `JUEGOS`}
...
//_allantilink.js
const smsTextoYT = () => { return 'FEDERICO - 𝗪𝗵𝗮𝘁𝘀𝗔𝗽𝗽 '} 
...
const eGataCoins = () => { return ' FedeCoins' }
const eGataTickers = () => { return 'Fede Tickers' } 
...
const smsWel2 = () => { return `SALUDO PARA LA LLEGADA AL GRUPO`}
...
const smsModAd2 = () => { return `Solo los Admins podrán USAR a federico en Grupos`}
...
const smsTex10 = () => { return '*MENU JUEGOS*'}
...
const smsSetB2 = () => { return `${lenguajeGB['smsAvisoIIG']()}*@user se va del grupo. Nadie lo va a extrañar*`}

//grupo-setwelcome.js
const smsSetW = () => { return `${lenguajeGB['smsAvisoEG']()}*LA BIENVENIDA DEL GRUPO HA SIDO CONFIGURADA*`}
const smsSetW2 = () => { return `${lenguajeGB['smsAvisoIIG']()}*@user se unió al grupo y no soy bueno con las bienvenidas*`}
...
//propietario(a).js
const smsJoin = (user) => { return `${packname}\n𝙀𝙎 𝙐𝙉 𝘽𝙊𝙏 𝘿𝙀 𝙒𝙃𝘼𝙏𝙎𝘼𝙋𝙋 𝙌𝙐𝙀 𝙏𝙀 𝘼𝙔𝙐𝘿𝘼𝙍𝘼 𝙍𝙀𝘼𝙇𝙄𝙕𝘼𝙍 𝘿𝙄𝙁𝙀𝙍𝙀𝙉𝙏𝙀𝙎 𝘼𝘾𝙏𝙄𝙑𝙄𝘿𝘼𝘿𝙀𝙎 🪄 𝘼𝙇 𝙋𝙍𝙄𝙑𝘼𝘿𝙊 𝙊 𝙂𝙍𝙐𝙋𝙊, 𝘿𝙄𝙎𝙁𝙍𝙐𝙏𝘼 𝘿𝙀 *FEDERICO*!!!\n\N *FEDERICO* 𝙁𝙐𝙀 𝙄𝙉𝙑𝙄𝙏𝘼𝘿 𝙋𝙊𝙍:\n@${user}`}
...
const smsJBInfo2 = () => { return `*La función es estable, sí presenta algún inconveniente Comuníquese al correo: calderonbeto2009@gmail.com*\n\n*Muchas Gracias por el apoyo a ${global.packname}*`}

//jadibot-deleteSesion.js
const smsJBDel = () => { return `${lenguajeGB['smsAvisoAG']()}*USE ESTE COMANDO AL BOT PRINCIPAL*`}
const smsJBAdios = () => { return `${lenguajeGB['smsAvisoEG']()}*NADIE TE VA A EXTRAÑAR ${global.packname} CHAOO!!*`}
...
const smsContacto2 = () => { return 'Soy Owner de ' + packname + ' sí tienes alguna duda me la puedes decir' }
const smsContacto3 = () => { return '👑 Owner'}
const smsContacto4 = () => { return 'Contacto Oficial de Federico '}
